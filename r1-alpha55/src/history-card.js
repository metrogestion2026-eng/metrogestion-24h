...
function renderHistoricalStage(stage, documentsByGroup, canEditDocuments, onDocumentsChanged) {
  const host = element('section', { className: 'a53-history-stage' });
  ...
  if ((stage.cancelado || stage.estado === 'anulada') && stage.motivo_cancelacion) {
    host.append(element('div', { className: 'notice danger', text: `T anulada: ${stage.motivo_cancelacion}` }));
  }

  host.append(createStageDocuments(stage, {
    canEdit: canEditDocuments,
    documents: documentsByGroup.get(stage.grupo_documental_id) || [],
