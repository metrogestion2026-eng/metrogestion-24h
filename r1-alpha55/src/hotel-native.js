...
async function renderHotelNative(container, access = {
  ...
  clear(container);
  container.dataset.alpha55HotelNative = 'loading';
  let editMode = false;
  let activeFilter = 'all';

  const headingActions = element('div', { className: 'hotel-heading-actions' }, [
    element('span', { className: 'badge', text: 'Hotel activo real · Alpha55' }),
  ]);
  const modeButton = access.editFicha
    ? element('button', {
        className: 'button secondary hotel-mode-button',
        type: 'button',
        text: '🔒 Modo lectura',
      })
    : null;
  ...
  if (!access.view) {
    ...
    container.dataset.alpha55HotelNative = '1';
    return;
  }
  ...
  if (boardResult.error) {
    container.append(notice(
      `Hotel se cargó, pero no pudo leerse la fecha de la pizarra: ${boardResult.error.message}`,
      'warning'
    ));
  ...
  if (editableResult.error) {
    container.append(notice(
      `Hotel se cargó, pero no pudo comprobarse la edición: ${editableResult.error.message}`,
      'warning'
    ));
  }
  ...
  try {
    ...
    container.append(notice(
      `Las fichas se cargaron, pero no pudo leerse la documentación: ${error.message}`,
      'warning'
    ));
  }
  loading.remove();
  ...
  const summary = element(
    ...
    HOTEL_FILTERS.map(filter => metric(
      filter,
      filter.states ? rows.filter(row => filter.states.has(row.estado)).length : rows.length
    ))
  );
  ...
  const renderRows = () => {
    ...
    if (access.editFicha) {
      modeNotice.append(editMode
        ? notice(
            `✏️ Lectura y edición activada · ${editableIds.size} fichas autorizadas. Los documentos de las T se gestionan desde su propio bloque.`,
            'warning'
          )
        : notice(
            '🔒 Protección de la ficha activada. Los documentos se añaden desde cada T, también arrastrándolos a su zona de carga.',
            'success'
          ));
    } else {
...
async function openNativeHotel(button) {
  ...
  try {
    ...
    content.append(notice(
      `No se pudo cargar Hotel: ${error?.message || 'error desconocido'}`,
      'danger'
    ));
    content.dataset.alpha55HotelNative = '1';
  } finally {
    rendering = false;
  }
}
...
function scheduleTakeover() {
  ...
  queueMicrotask(() => {
    ...
    if (!button || content.dataset.alpha55HotelNative === '1' || !content.querySelector('.summary-grid')) return;
