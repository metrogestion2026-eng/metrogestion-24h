...
test('solo el fondo blanco de H origina necesidades, incluso si ya estaban vinculadas', () => {
  const values = [
    Array(17).fill(''),
    row({ dfm: 'R1487', taller: 'DIRECAUTO', tipo: 'REPARACIÓN', designacion: 'GP', necesidad: '06/09/2026' }),
    row({ dfm: 'R1487', taller: 'DIRECAUTO', tipo: 'REPARACIÓN', designacion: 'AV', necesidad: '06/09/2026' }),
    row({ dfm: 'R1487', parada: 'PA-2600152', taller: 'DIRECAUTO', tipo: 'MANTENIMIENTO', designacion: 'BPW', necesidad: '06/09/2026' }),
  ];
...
test('el amarillo en A incluye una necesidad aunque esté a más de un mes', () => {
  const values = [
    Array(17).fill(''),
    row({ dfm: 'R1443', taller: 'FRIDIEL', tipo: 'AVERÍA', designacion: 'AV', necesidad: '08/10/2026' }),
    row({ dfm: 'R1443', taller: 'FRIDIEL', tipo: 'TRÁMITE', designacion: 'LKT', necesidad: '08/10/2026' }),
  ];
  const notes = [[''], [''], ['']];
...
test('una asignación antigua ya vinculada se omite y no bloquea las pendientes actuales', () => {
  ...
  const applied = context.metrogestionAplicarAsignacionesTrabajos_(sheet, [
    ...
    { fila: 3, clave_fila: context.metrogestionClaveFilaTrabajo_(pending) },
  ]);
...
test('varios bloques del mismo trabajo pueden compartir vínculo dentro de la misma parada', () => {
  ...
  const applied = context.metrogestionAplicarAsignacionesTrabajos_(sheet, [
    ...
    { fila: 2, clave_fila: context.metrogestionClaveFilaTrabajo_(first) },
    { fila: 3, clave_fila: context.metrogestionClaveFilaTrabajo_(second) },
  ]);
