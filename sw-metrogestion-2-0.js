// Capa final v37: fuerza actualización y conserva wrapper estable + correcciones visuales + edición individual por ficha.
importScripts('./sw-metrogestion-v36-base.js?v=37');
...
self.fetch=async(input,init)=>{
  ...
  try{
    ...
    if(response.ok&&url.origin===self.location.origin&&url.pathname.endsWith('/metrogestion-2-0.html')){
      ...
      html=html.replace("const canEditHotel = () => hasHotelEditPermission() && !hotelReadMode;","const canEditHotel = () => hasHotelEditPermission() && (!hotelReadMode || Boolean(hotelEditingUnitId));");
      ...
      html=html.replace("          ${stopHeading}\\n          ${stagesHeading}\\n          ${stages}","          ${stagesHeading}\\n          ${stages}");
      html=html.replace(
      ...
      html=html.replace("const showHotelSaved=(message='✓ Cambios guardados correctamente')=>{","const showHotelSaved=(message='✓ Cambios guardados correctamente')=>{hotelEditingUnitId='';hotelReadMode=true;");
  ...
  }catch(error){console.warn('No se pudieron aplicar las correcciones visuales v37',error);}
  return response;
};
