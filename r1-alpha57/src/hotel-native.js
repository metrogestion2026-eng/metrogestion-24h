...
async function loadHotelData(access) {
  ...
  if (recordIds.length) {
    const { data, error } = await supabase
      .from('etapas_hotel')
      .select('id,registro_hotel_id,seguimiento_id,grupo_documental_id,nombre,posicion,estado,tipo_etapa,taller_id,centro_taller_id,lugar,fecha_prevista,fecha_inicio_real,fecha_fin_real,fecha_real,observaciones,cancelado,motivo_cancelacion,version,accion_sistema,etapa_origen_id,actualizado_en')
...
async function renderHotelNative(container, access) {
  ...
  const headingActions = element('div', { className: 'hotel-heading-actions' }, [
    element('span', { className: 'badge', text: 'Hotel activo real · Alpha57' }),
  ]);
    ...
    const renderRows = () => {
      ...
      if (access.editFicha) {
        modeNotice.append(editMode
          ? notice(`✏️ Lectura y edición activada · ${editableIds.size} fichas autorizadas. En “Trabajos de esta T” puedes editar o crear tipos.`, 'warning')
          : notice('🔒 Protección de la ficha activada. Pulsa “Ver ficha” en cualquier T para abrirla siempre en modo consulta.', 'success'));
      } else {
