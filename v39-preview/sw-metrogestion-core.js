const CACHE_NAME = 'gestion-metrogestion-2-0-v36-auto-update-1';
...
const patchActivationHtml = html => {
  // Activar 24H: no crear borradores automáticos salvo retorno técnico de llamada.
  html = html.replace(
  ...
  // Hotel: confirmación, deshacer, calendario, aviso histórico, modo lectura y cambios desde ayer.
  html = html.replace(
  ...
  const hotelHandlers = `root.querySelector('#hotel-search').addEventListener('input', renderHotel);
      ...
      const showHotelSaved=(message='✓ Cambios guardados correctamente')=>{
        ...
        const text=root.querySelector('#hotel-save-confirmation-text');
        if(!notice)return;
        clearTimeout(hotelSaveNoticeTimer);
        if(text)text.textContent=message;
        notice.classList.remove('hidden');
        try{notice.scrollIntoView({behavior:'smooth',block:'nearest'});}catch{}
        hotelSaveNoticeTimer=setTimeout(()=>notice.classList.add('hidden'),8000);
      };
      root.querySelector('#hotel-undo-last')?.addEventListener('click',async()=>{
        if(!hasHotelEditPermission()){window.alert('Este usuario no tiene permiso para modificar el Hotel.');return;}
        if(!window.confirm('¿Deshacer tu última modificación del Hotel? Solo se revertirá si nadie ha modificado después ese mismo dato.'))return;
        const button=root.querySelector('#hotel-undo-last');if(button)button.disabled=true;
        const {data,error}=await supabaseClient.rpc('deshacer_ultima_accion_hotel');
        if(button)button.disabled=false;
        if(error){window.alert('No se pudo deshacer: '+error.message);return;}
        showHotelSaved('↶ '+(data||'Última acción deshecha'));
      ...
      const refreshHotelReadMode=()=>{
        ...
        if(!allowed){hotelReadMode=true;checkbox.checked=true;checkbox.disabled=true;if(help)help.textContent='Modo lectura permanente · este usuario no tiene permiso de edición.';}
        else{checkbox.disabled=false;checkbox.checked=hotelReadMode;if(help)help.textContent=hotelReadMode?'Protección activada: no se pueden modificar datos.':'⚠ EDICIÓN ACTIVADA · revisa la fecha antes de guardar cambios.';}
      };
      ...
      const loadHotelChangesFromPrevious=async()=>{
        ...
        const currentMap=new Map(currentRows.map(row=>[key(row),row])),previousMap=new Map(previousRows.map(row=>[key(row),row]));
        const changes=[];
        currentRows.forEach(row=>{
          const before=previousMap.get(key(row));
          if(!before){changes.push({row,title:'Nuevo movimiento',detail:'No estaba en la pizarra anterior.',kind:'new'});return;}
          const parts=[];
          ...
          (before.etapas_hotel||[]).forEach(s=>{if(!cm.has(sk(s)))parts.push('T retirada: '+s.nombre);});
          if(parts.length)changes.push({row,title:'Cambios detectados',detail:parts.join(' · '),kind:'change'});
        });
  ...
  // Confirmaciones visuales después de guardar.
  ...
  // Al entrar al Hotel siempre vuelve a modo lectura.
  html=html.replace("if (next === 'hotel') {","if (next === 'hotel') { hotelReadMode=true; setTimeout(() => { try { refreshHotelReadMode(); renderHotel(); } catch {} },0);");

  // NUEVO SISTEMA DE ACTUALIZACIÓN: sin caché, comprobación frecuente y automática.
  html=html.replace(
    "navigator.serviceWorker.register('sw-metrogestion-2-0.js?v=36')",
    "navigator.serviceWorker.register('sw-metrogestion-2-0.js?v=36-auto-1', {updateViaCache:'none'})"
  );
  html=html.replace(
...
const fetchPatchedHtml=async request=>{
  ...
  const patched=new Response(text,{status:response.status,statusText:response.statusText,headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'}});
...
self.addEventListener('install',event=>{
  // La versión nueva no queda esperando: se activa automáticamente.
  self.skipWaiting();
  event.waitUntil((async()=>{
    const cache=await caches.open(CACHE_NAME);
    await cache.addAll(APP_FILES.filter(f=>!f.endsWith('.html')));
    try{await fetchPatchedHtml(new Request('./metrogestion-2-0.html',{cache:'no-store'}));}catch{}
  })());
});
...
self.addEventListener('activate',event=>event.waitUntil(
  caches.keys()
    .then(keys=>Promise.all(keys.filter(key=>(key.startsWith('gestion-24h-')||key.startsWith('gestion-metrogestion-'))&&key!==CACHE_NAME).map(key=>caches.delete(key))))
...
self.addEventListener('fetch',event=>{
  ...
  const allowed=APP_FILES.map(f=>new URL(f,self.location.href).pathname);
  if(!allowed.includes(u.pathname))return;
  if(u.pathname.endsWith('/metrogestion-2-0.html')){
    event.respondWith(fetchPatchedHtml(event.request).catch(()=>caches.match(event.request)));
    return;
  }
  event.respondWith(caches.match(event.request).then(c=>c||fetch(event.request,{cache:'no-store'})));
});