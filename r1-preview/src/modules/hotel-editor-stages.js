...
function renderWorks(detail, stage, worksHost, markDirty) {
  ...
  stage.trabajos.forEach((work, workIndex) => {
    ...
    const typeOptions = detail.catalogos.tipos_trabajo.map(item => [
      item.codigo,
      `${item.codigo} · ${item.nombre}`
    ]);
    const workType = createSelect(typeOptions, work.tipo_trabajo || typeOptions[0]?.[0] || '');
  ...
  const renderStages = () => {
    ...
    detail.etapas.forEach((stage, stageIndex) => {
      ...
      stageCard.append(
        ...
        stageCancelBox,
        element('div', { className: 'editor-subheading' }, [element('strong', { text: 'Trabajos de esta T' })]),
        worksHost,
        addWorkButton
      );
