...
function renderNavigation(profile) {
  clear(dom.moduleNav);
  const adminOnlyModules = new Set(['importacion_hotel', 'transformacion_hotel', 'revision_hotel']);
  const visibleModules = MODULES.filter(module => {
    if (adminOnlyModules.has(module.id)) return profile?.tipo_usuario === 'administrador_principal';
    return getModuleAccess(profile, module.id).view;
  });
  ...
  visibleModules.forEach(module => {
    ...
    const button = element('button', {
      ...
      title: module.id === 'revision_hotel'
        ? 'Revisión aislada · no importa datos'
        : adminOnly
          ? 'Vista previa de preparación · solo lectura'
          : access.edit ? 'Permiso de edición' : 'Solo lectura'
    });
