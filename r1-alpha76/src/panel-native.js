...
import { openHotelEditor } from './hotel-editor.js?v=76.1';
...
function hotelItem(row, latestByHotel) {
  ...
  return {
    ...
    module: 'hotel',
    recordId: row.id,
    searchQuery: row.dfm || row.numero_parada || row.matricula || '',
...
async function renderPanel({ automatic = false } = {}) {
  ...
  try {
    ...
    const activatePanelItem = (item, spec = {}) => {
      ...
      if (moduleId === 'hotel' && !canEditHotel) {
        sessionStorage.setItem('alpha76HotelSearch', item?.searchQuery || item?.title || '');
      }
