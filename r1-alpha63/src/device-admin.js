...
function ensureStyle() {
  if (document.querySelector('#alpha63-device-style')) return;
  const style = document.createElement('style');
  style.id = 'alpha63-device-style';
  style.textContent = `
...
async function changeDevice(section, device, authorize, status) {
  ...
  if (authorize) {
    ...
    if (!accepted) return;
    observations = `Autorizado como ${definition.label.toLocaleLowerCase('es-ES')} desde Usuarios · Alpha63`;
  } else {
...
function renderDeviceRecord(section, device, { authorizedForType, status, historical = false }) {
  const card = el('article', null, historical ? 'a63-device-request' : 'a63-device-request');
  const definition = typeDefinition(device.tipo_dispositivo);
  card.append(
...
async function renderDeviceSection(section, successMessage = '') {
  ...
  section.dataset.alpha63DeviceSection = 'loading';
  ...
  const note = el(
    'div',
    'El usuario debe entrar una vez desde cada equipo. La solicitud aparecerá aquí y podrás autorizar por separado el móvil y el ordenador.',
    'a63-device-note'
  );
...
function syncDeviceSection() {
  ...
  if (!section) return;
  if (section.dataset.alpha63DeviceSection) return;
