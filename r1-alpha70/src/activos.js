...
const MODULE_FLAG = 'alpha70Activos';
...
function renderTable(host, rows, access, onChanged) {
  ...
  rows.forEach(row => {
    ...
    operational.append(
      ...
      el('span', row.fecha_alta_manteniment ? `Alta ${formatDate(row.fecha_alta_manteniment)}` : 'Sin fecha de alta', 'a70-assets-subline')
    );
...
function openAssetModal(row, onChanged) {
  ...
  form.append(
    ...
    inputField('Fecha de matriculación', { name: 'fecha_matriculacion', value: row?.fecha_matriculacion, type: 'date' }),
    inputField('Fecha de alta', { name: 'fecha_alta_manteniment', value: row?.fecha_alta_manteniment || new Date().toISOString().slice(0, 10), type: 'date', required: true }),
    ...
    inputField('Fin de contrato', { name: 'fin_contrato_fecha', value: row?.fin_contrato_fecha, type: 'date' }),
...
async function loadAssets(root, profile, sequence) {
  ...
  const { data, error } = await supabase
    .from('vehiculos')
    .select('id,dfm,matricula,categoria,clase_vehiculo,tipo_motor,tipo_manteniment,marca,modelo,bastidor,upc,telefono,fecha_matriculacion,fecha_alta_manteniment,fin_contrato_fecha,fin_contrato_km,km_actual,contrato_texto,asignacion_manteniment,reserva,activo,version,alta_manual_en,baja_manual_en,motivo_baja,baja_manual_bloquea_sync,fuente_manteniment_fila,actualizado_en')
