-- Resuelve las T desde la ficha más reciente y conserva visitas agrupadas.
...
AS $function$
  ...
  join public.etapas_hotel actual
   ...
   and actual.grupo_documental_id = origen.grupo_documental_id
  ...
  limit 1;
$function$;
REVOKE ALL ON FUNCTION app_private.manteniment_etapa_actual(uuid, uuid) FROM PUBLIC, anon, authenticated;
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'fecha_entrada', coalesce(w.fecha_realizada, (coalesce(etapa.fecha_real, etapa.fecha_fin_real) at time zone 'Europe/Madrid')::date),
