...
-- Sin cambios de permisos, datos históricos ni políticas de acceso.
-- Solo distingue realización administrativa de salida/colocación física.
create or replace function app_private.manteniment_fecha_cierre_necesidad(
  p_tipo text, p_j date, p_k date
...
$function$;
revoke all on function app_private.manteniment_fecha_cierre_necesidad(text,date,date) from public, anon, authenticated;
    ...
    loop
      ...
      loop
        ...
        if v_visit_stage.modalidad in ('taller', 'entrada_sin_recogida') then
          ...
          if v_stage_id is null then
            ...
            insert into public.etapas_hotel(
              ...
              v_pos,
              case when app_private.manteniment_fecha_cierre_necesidad(v_work.designacion, v_work.fecha_realizada, v_work.fecha_recogida) is null then 'pendiente' else 'realizada' end,
              case when app_private.manteniment_fecha_cierre_necesidad(v_work.designacion, v_work.fecha_realizada, v_work.fecha_recogida) is null then 'pendiente' else 'realizada' end,
              'otro', v_work.taller, null,
              case when app_private.manteniment_fecha_cierre_necesidad(v_work.designacion, v_work.fecha_realizada, v_work.fecha_recogida) is null then null else app_private.manteniment_fecha_cierre_necesidad(v_work.designacion, v_work.fecha_realizada, v_work.fecha_recogida)::timestamp at time zone 'Europe/Madrid' end,
              case when app_private.manteniment_fecha_cierre_necesidad(v_work.designacion, v_work.fecha_realizada, v_work.fecha_recogida) is null then null else app_private.manteniment_fecha_cierre_necesidad(v_work.designacion, v_work.fecha_realizada, v_work.fecha_recogida)::timestamp at time zone 'Europe/Madrid' end,
        ...
        if v_visit_stage.modalidad not in ('taller', 'entrada_sin_recogida')
           and app_private.manteniment_fecha_cierre_necesidad(v_work.designacion, v_work.fecha_realizada, v_work.fecha_recogida) is not null then
          update public.etapas_hotel
          set estado = 'realizada',
              estado_catalogo_codigo = 'realizada',
              fecha_fin_real = coalesce(fecha_fin_real, app_private.manteniment_fecha_cierre_necesidad(v_work.designacion, v_work.fecha_realizada, v_work.fecha_recogida)::timestamp at time zone 'Europe/Madrid'),
              fecha_real = coalesce(fecha_real, app_private.manteniment_fecha_cierre_necesidad(v_work.designacion, v_work.fecha_realizada, v_work.fecha_recogida)::timestamp at time zone 'Europe/Madrid'),
              modificado_por = coalesce(v_actor, modificado_por)
...
begin
  ...
  with pendientes as (
    ...
    where app_private.manteniment_fecha_cierre_necesidad(mt.designacion, mt.fecha_realizada, mt.fecha_recogida) is null
  )
  select
    p.necesidad_id,
    p.etapa_id,
