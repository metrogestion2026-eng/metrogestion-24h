...
-- Alpha74.16: los trámites mandan sobre F, LKT/EXTINTOR no se duplican y
-- TM conserva entrada con trabajos, pero nunca genera recogida.
    ...
    if v_visit.id is null then
      ...
      set modalidad = v_modalidad,
          ...
          grupo_recogida_id = case
            when v_modalidad = 'taller' then grupo_recogida_id
            else null
          end,
...
-- La aplicación puede reconciliar entradas creadas manualmente. Esta protección
-- impide que ese proceso vuelva a crear una recogida para TM o para un trámite.
        ...
        exists (
          ...
          where v.grupo_entrada_id = origen.grupo_documental_id
            and (
              ...
              or regexp_replace(upper(btrim(coalesce(v.taller, ''))), '[[:space:]]+', ' ', 'g') = 'TM'
            )
        )
        or exists (
          select 1
...
revoke all on function app_private.etapa_manteniment_sin_recogida_alpha74(uuid)
  from public, anon, authenticated;
grant execute on function app_private.etapa_manteniment_sin_recogida_alpha74(uuid)
  to service_role;

create or replace function app_private.bloquear_recogida_manteniment_alpha74()
returns trigger
language plpgsql
security definer
set search_path = pg_catalog, public, app_private
as $function$
begin
  if new.tipo_etapa = 'recogida_taller'
     and new.etapa_origen_id is not null
     and app_private.etapa_manteniment_sin_recogida_alpha74(new.etapa_origen_id) then
    return null;
  end if;
  return new;
end;
...
revoke all on function app_private.bloquear_recogida_manteniment_alpha74()
  from public, anon, authenticated;

drop trigger if exists etapas_hotel_bloquear_recogida_manteniment_alpha74
  on public.etapas_hotel;
create trigger etapas_hotel_bloquear_recogida_manteniment_alpha74
before insert on public.etapas_hotel
for each row execute function app_private.bloquear_recogida_manteniment_alpha74();

-- Corrige las visitas ya vinculadas sin cambiar su identidad ni número de actuación.
update app_private.manteniment_t_visitas v
set modalidad = 'entrada_sin_recogida',
    grupo_recogida_id = null,
    actualizado_en = clock_timestamp()
where v.modalidad = 'taller'
  and regexp_replace(upper(btrim(coalesce(v.taller, ''))), '[[:space:]]+', ' ', 'g') = 'TM';

update app_private.manteniment_t_visitas v
set modalidad = 'tramite',
    grupo_entrada_id = null,
    grupo_recogida_id = null,
    actualizado_en = clock_timestamp()
where v.modalidad <> 'tramite'
  and exists (
    select 1
    from app_private.manteniment_t_trabajos w
    where w.visita_id = v.id
      and (
        upper(btrim(w.designacion)) in ('LKT', 'EXTINTOR')
        or translate(upper(coalesce(w.tipo_trabajo, '')), 'ÁÉÍÓÚÜÑ', 'AEIOUUN') like '%TRAMITE%'
      )
  )
  and not exists (
    select 1
    from app_private.manteniment_t_trabajos w
    where w.visita_id = v.id
      and upper(btrim(w.designacion)) not in ('LKT', 'EXTINTOR')
      and translate(upper(coalesce(w.tipo_trabajo, '')), 'ÁÉÍÓÚÜÑ', 'AEIOUUN') not like '%TRAMITE%'
  );

-- Convierte las T de trámite antiguas para que no sean interpretadas como entradas.
...
-- Unifica un LKT generado con la única T LKT ya existente en la misma actuación.
do $cleanup_lkt$
declare
  x record;
begin
  for x in
    select
      w.id as necesidad_id,
      w.trabajo_hotel_id,
      generada.id as generada_id,
      legado.id as legado_id,
      legado.grupo_documental_id as legado_grupo,
      w.taller,
      w.fecha_necesidad
    from app_private.manteniment_t_trabajos w
    join public.trabajos_etapa_hotel t on t.id = w.trabajo_hotel_id and not t.cancelado
    join public.etapas_hotel generada on generada.id = t.etapa_hotel_id and not generada.cancelado
    join lateral (
      select e.id, e.grupo_documental_id
      from public.etapas_hotel e
      where e.registro_hotel_id = generada.registro_hotel_id
        and e.id <> generada.id
        and not e.cancelado
        and regexp_replace(
              translate(upper(btrim(coalesce(e.nombre, ''))), 'ÁÉÍÓÚÜÑ', 'AEIOUUN'),
              '^TRAMITE[[:space:]·:.-]*', '', 'g'
            ) = 'LKT'
      order by e.posicion, e.creado_en
      limit 1
    ) legado on true
    where upper(btrim(w.designacion)) = 'LKT'
      and translate(upper(coalesce(w.tipo_trabajo, '')), 'ÁÉÍÓÚÜÑ', 'AEIOUUN')
            like any (array['%TRAMITE%', '%GESTION%'])
      and translate(upper(coalesce(generada.nombre, '')), 'ÁÉÍÓÚÜÑ', 'AEIOUUN')
            ~ '(^|[^A-Z0-9])(TRAMITE|GESTION)([^A-Z0-9]|$)'
  loop
    update public.trabajos_etapa_hotel
    set etapa_hotel_id = x.legado_id
    where id = x.trabajo_hotel_id;

    update app_private.manteniment_t_trabajos
    set grupo_etapa_id = x.legado_grupo,
        tipo_trabajo = 'TRÁMITE',
        actualizado_en = clock_timestamp()
    where id = x.necesidad_id;

    update public.etapas_hotel
    set nombre = 'Trámite · LKT',
        tipo_etapa = 'otro',
        lugar = coalesce(nullif(x.taller, ''), lugar),
        observaciones = case
          when btrim(coalesce(observaciones, '')) = ''
            then 'Necesidad detectada en MANTENIMENT el ' || to_char(x.fecha_necesidad, 'DD/MM/YYYY') || '.'
          else observaciones
        end,
        modificado_por = coalesce(modificado_por, creado_por)
    where id = x.legado_id;

    if not exists (
      select 1 from public.trabajos_etapa_hotel t
      where t.etapa_hotel_id = x.generada_id and not t.cancelado
    ) then
      update public.etapas_hotel
      set cancelado = true,
          estado = 'anulada',
          estado_catalogo_codigo = 'anulada',
          motivo_cancelacion = 'T duplicada al interpretar LKT como Gestión; unificada con la T de Trámite.',
          cancelado_en = clock_timestamp(),
          cancelado_por = coalesce(modificado_por, creado_por),
          modificado_por = coalesce(modificado_por, creado_por)
      where id = x.generada_id and not cancelado and estado <> 'realizada';
    end if;
  end loop;
end;
$cleanup_lkt$;

-- Retira únicamente recogidas pendientes que contradicen las reglas anteriores.
update public.etapas_hotel pickup
set cancelado = true,
    estado = 'anulada',
    estado_catalogo_codigo = 'anulada',
    motivo_cancelacion = 'TM y los trámites LKT/EXTINTOR no generan recogida.',
    cancelado_en = clock_timestamp(),
    cancelado_por = coalesce(pickup.modificado_por, pickup.creado_por),
    modificado_por = coalesce(pickup.modificado_por, pickup.creado_por)
where not pickup.cancelado
  and pickup.tipo_etapa = 'recogida_taller'
  and pickup.estado <> 'realizada'
  and pickup.etapa_origen_id is not null
  and app_private.etapa_manteniment_sin_recogida_alpha74(pickup.etapa_origen_id);
