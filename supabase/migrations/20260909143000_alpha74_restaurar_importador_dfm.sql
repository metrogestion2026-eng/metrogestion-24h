...
-- Recupera el importador Alpha74 más reciente antes de aplicar la inferencia DFM.
-- La ampliación DFM debe envolver esta versión, no la base histórica anterior.
...
begin
  ...
  return app_private.manteniment_importar_trabajos_alpha74_base(v_trabajos);
end;
$function$;
...
comment on function app_private.manteniment_importar_trabajos(jsonb) is
  'Normaliza el taller implícito de R y DFM y delega en el importador Alpha74 vigente.';

-- En una reconciliación automática el estado operativo puede cambiar antes de
-- que una etiqueta visible personalizada sea sustituida. El proceso automático
-- debe recuperar el catálogo estándar del nuevo estado; el editor mantiene la
-- validación estricta de las etiquetas elegidas por el usuario.
...
declare
  ...
  v_reconciliacion_automatica boolean;
begin
  begin
    v_context_id := nullif(
    ...
    v_reconciliacion_automatica :=
      coalesce(current_setting('app.reconciliando_etapas', true), '') = '1'
      or coalesce(current_setting('app.audit_origin', true), '') like 'manteniment-alpha74-%';
  exception when others then
    v_context_id := null;
    v_payload := '[]'::jsonb;
    v_editor_save := false;
    v_reconciliacion_automatica := false;
  end;
  ...
  if (v_operativo is null or v_operativo <> new.estado)
    and v_reconciliacion_automatica
    and v_codigo is null then
    new.estado_catalogo_codigo := new.estado;
    select c.estado_operativo
      into v_operativo
    from public.catalogo_estados_etapa_hotel c
...
comment on function app_private.sincronizar_catalogo_estado_etapa() is
  'Valida el estado visible y realinea su catálogo solo durante reconciliaciones automáticas de MANTENIMENT.';
