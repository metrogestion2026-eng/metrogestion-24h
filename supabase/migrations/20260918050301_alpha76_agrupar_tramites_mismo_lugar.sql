DO $guard$ BEGIN
  IF md5(rtrim(pg_get_functiondef('app_private.manteniment_importar_trabajos_alpha74_base(jsonb)'::regprocedure), E' \t\n\r')) <> 'bc7bf160233215f262e6bced7b554101' THEN
    RAISE EXCEPTION 'El importador cambió durante la revisión; no se sustituye';
  END IF;
END $guard$;

-- Agrupar trabajos por lugar y modalidad; conservar las T agrupadas manualmente.
...
begin
  ...
  loop
    ...
    v_clave_visita := case
      ...
      when v_modalidad in ('tramite', 'gestion') and coalesce(v_group.taller_norm, '') <> ''
        then upper(v_modalidad) || '|F:' || v_group.taller_norm
    ...
    if v_modalidad in ('tramite', 'gestion') and coalesce(v_group.taller_norm, '') <> '' then
      ...
      if v_current_work.trabajo_hotel_id is not null then
        ...
        join public.etapas_hotel e on e.id = app_private.manteniment_etapa_actual(
          v_group.seguimiento_id, t.etapa_hotel_id)
        where t.id = v_current_work.trabajo_hotel_id and e.tipo_etapa = 'otro';
      end if;
      update app_private.manteniment_t_visitas
