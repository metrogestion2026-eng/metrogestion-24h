...
-- Alpha74.13: F manda sobre G, ventana móvil de un mes, prioridad amarilla
-- en A y reutilización segura de entradas de taller ya existentes en la ficha.
...
begin
  ...
  loop
    ...
    if not found then
      ...
      -- recogida. Se reutilizan si existe una única entrada compatible, evitando
      -- crear una segunda T para el mismo taller.
      if v_modalidad = 'taller' then
        ...
        where e.registro_hotel_id = v_group.registro_id
          ...
          and regexp_replace(
                upper(btrim(concat_ws(' ', e.nombre, e.lugar))),
                '[[:space:]]+', ' ', 'g'
              ) like '%' || v_group.taller_norm || '%';
