...
function renderNavigation(profile) {
  ...
  const visibleModules = MODULES.filter(module => {
    if (module.id === 'importacion_hotel') return profile?.tipo_usuario === 'administrador_principal';
    return getModuleAccess(profile, module.id).view;
  });

  visibleModules.forEach(module => {
    const access = module.id === 'importacion_hotel'
      ? { view: true, edit: false }
      : getModuleAccess(profile, module.id);
    const button = element('button', {
      className: 'button secondary',
      type: 'button',
      text: `${module.icon} ${module.label}`,
      dataset: { module: module.id },
      title: module.id === 'importacion_hotel'
        ? 'Vista previa de importación · solo lectura'
