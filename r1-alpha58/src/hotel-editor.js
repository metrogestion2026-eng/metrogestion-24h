...
function savedMessage(saved) {
  ...
  const newTypes = Number(saved?.catalogos_nuevos?.tipos_trabajo || 0);
  const parts = [];

  if (auditEvents) parts.push(`${auditEvents} cambio${auditEvents === 1 ? '' : 's'} auditado${auditEvents === 1 ? '' : 's'}`);
  else parts.push('sin cambios adicionales que auditar');

  if (newTypes) {
    parts.push(`${newTypes} tipo${newTypes === 1 ? '' : 's'} nuevo${newTypes === 1 ? '' : 's'} añadido${newTypes === 1 ? '' : 's'} al listado`);
  }
...
export async function openHotelEditor(registroId, { onSaved } = {}) {
  ...
  saveButton.addEventListener('click', async () => {
    ...
    status.className = 'hotel-editor-status';
    status.textContent = 'Guardando la ficha y actualizando el listado de tipos de trabajo…';

    const saveRequestId = requestId();
    const { data: saved, error: saveError } = await supabase.rpc('guardar_ficha_hotel_edicion_catalogos', {
    ...
    await onSaved?.();
    saveButton.textContent = Number(saved?.catalogos_nuevos?.tipos_trabajo || 0)
      ? 'Guardado · listado actualizado'
  ...
  status.textContent = vehiclesResult.error
    ...
    : `Ficha cargada. Hay ${detail.catalogos.tipos_trabajo.length} tipos disponibles; también puedes escribir uno nuevo. Nada se modifica hasta guardar.`;
}
