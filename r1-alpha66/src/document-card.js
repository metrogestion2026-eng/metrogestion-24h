...
import { openDocumentShare } from './document-share.js';
...
export function documentCard(doc,canEdit,reload){
  ...
  open.type=download.type=history.type='button';open.addEventListener('click',()=>openStoredDocument(doc));download.addEventListener('click',()=>downloadStoredDocument(doc));history.addEventListener('click',()=>showDocumentHistory(doc));actions.append(open,download);
  if(!doc.cancelado){const share=el('button','Compartir','button primary compact');share.type='button';share.title='Compartir el archivo o un enlace temporal';share.addEventListener('click',()=>openDocumentShare(doc));actions.append(share);}
