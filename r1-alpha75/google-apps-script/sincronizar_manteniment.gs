...
function metrogestionVerEstadoLocal() {
  ...
  const diagnostic = metrogestionDiagnosticoSincronizacion();
  SpreadsheetApp.getUi().alert([
    ...
    `Último paso: ${diagnostic.paso} (${diagnostic.fecha || 'sin fecha'})`,
  ].join('\n'));
}

function metrogestionSolicitarCiclo_(sheet, modo, token) {
metrogestionPrepararHoja_(sheet);
...
const pendientes = metrogestionLeerPendientesListado_(values, workNotes, workBackgrounds, orderBackgrounds);
...
payload.checksum = metrogestionSha256_(JSON.stringify({ filas: rows, paradas, trabajos, pendientes }));
...
function metrogestionPrepararHoja_(sheet) {
  // Se retira temporalmente el filtro para poder convertir matrices internas
...
function metrogestionRestaurarFiltro_(sheet, state) {
  ...
  const dataRows = Math.max(1, sheet.getLastRow() - state.startRow + 1);
  // Nunca dejar fuera del filtro filas con datos ya existentes. También
  // conserva un rango anterior más largo si el usuario lo había preparado así.
  const numRows = Math.min(availableRows, Math.max(state.numRows, dataRows));
  if (numRows < 1) return;
  const filter = sheet
...
function metrogestionCantidadMantenimiento_(value) {
  if (typeof value === 'number') return Number.isFinite(value) && value > 0 ? value : null;
  let text = metrogestionNormalizar_(value).replace(/\s*(?:KM|HORAS?|H)$/, '').trim();
...
function metrogestionDetalleVehiculo_(value) {
  return /^(?:T\d+|M\d+(?:\b|\()|R\d+|EO(?:\b|\+)|EP\d+|FRENOS|RETARDADOR|CAMBIO|DIFERENCIAL|AF$|FF$|REFRIGERANTE)/.test(metrogestionNormalizar_(value));
}

// P en PARADA es kilometraje facturable: nunca se usa como lectura del odómetro.
// Prioridad: telemática, trabajo realizado en taller del vehículo, previsto conocido.
function metrogestionReferenciasKilometros_(values) {
  const refs = new Map();
  values.forEach(row => {
    const dfm = metrogestionNormalizar_(row[0]);
    const tipo = metrogestionTipoNecesidad_(row[7]);
    const taller = metrogestionNormalizar_(row[5]);
    const cantidad = metrogestionCantidadMantenimiento_(row[15]);
    if (!dfm || !cantidad || /(?:HORAS?|H)$/.test(metrogestionNormalizar_(row[15]))) return;
    const telematica = ['TRUCKPOINT', 'IVECO ON', 'GESINFLOT'].includes(tipo);
    const realizado = ['MCD', 'AV'].includes(tipo) && String(row[9] || '').trim()
      && /^(?:AUTODIS|AUTO DISTRIBUCION|STERN(?: MOTOR)?|MAN|VOLVO(?: (?:24H|ZO|ZF))?|REUS FLEMING|FLEMING|SIDECO)$/.test(taller);
    const previsto = tipo === 'MCD' && metrogestionDetalleVehiculo_(row[16]);
    const prioridad = telematica ? 3 : realizado ? 2 : previsto ? 1 : 0;
    if (!prioridad) return;
    let fecha = '';
    try { fecha = metrogestionFechaNecesidad_(row[9] || row[8]) || ''; } catch (_) { /* sin fecha fiable */ }
    const anterior = refs.get(dfm);
    if (!anterior || prioridad > anterior.prioridad || (prioridad === anterior.prioridad
        && (prioridad === 1 ? cantidad < anterior.cantidad : fecha >= anterior.fecha))) {
      refs.set(dfm, { cantidad, prioridad, fecha });
    }
  });
...
function metrogestionReglaTallerPendiente_(row, referenciasKm = new Map()) {
  if (!String(row[0] || '').trim()
      || String(row[9] || '').trim() || String(row[10] || '').trim()) return null;
  ...
  if (['ATP', 'TMG'].includes(designacion)) return { taller: 'INVERYCA' };
  const vehiculo = /MERCEDES|BENZ|^MER(?:\/|$)/.test(marca) ? 'STERN MOTOR'
    : /IVECO|^IV\//.test(marca) ? 'AUTODIS'
    : /(^|[^A-Z])MAN([^A-Z]|$)/.test(marca) ? 'MAN'
    : /VOLVO/.test(marca) ? 'VOLVO' : '';
  const frio = /CARRIER|\/CA(?:R{0,2})?$|\/C$/.test(marca) ? 'FRIDIEL'
    : /THER?MO|TERMO/.test(marca) ? 'FRIGICOLL'
    : /HW|HWASUNG/.test(marca) ? 'DIESEL PENEDÈS' : '';
  const frigorifico = !!frio || /FRIG/.test(marca);
  // GP/GC de Iveco van a Autodis también en rígidos con equipo de frío.
  if (vehiculo === 'AUTODIS' && ['GP', 'GC'].includes(designacion)) return { taller: 'AUTODIS' };
  if (frigorifico && ['GP', 'GC'].includes(designacion)) return { taller: 'DIRECAUTO' };
  if (!['MCD', 'AV'].includes(designacion)) return null;
  if (designacion === 'AV' && /\bFRAGADIS\b/.test(metrogestionNormalizar_(row[13]))) {
    if (vehiculo === 'AUTODIS') return { taller: 'SIDECO' };
    if (vehiculo === 'VOLVO') return { taller: 'REUS FLEMING' };
  }
  if (!vehiculo) return frio ? { taller: frio } : null;
  if (!marca.includes('/') && !frio) return { taller: vehiculo };
  const delVehiculo = metrogestionDetalleVehiculo_(detalle);
  const delFrio = /^(?:[AB](?:\b|\d)|\d+\s*H\b|MHW\b|KIT\s*[AB]\b)|HWASUNG|CARRIER|THER?MO|TERMO|FRIGO/.test(detalle);
  if (delVehiculo) return { taller: vehiculo };
  if (delFrio) return frio ? { taller: frio } : null;
  if (designacion === 'MCD') {
    const unidad = metrogestionNormalizar_(row[15]);
    const cantidad = metrogestionCantidadMantenimiento_(row[15]);
    const km = referenciasKm.get(metrogestionNormalizar_(row[0]));
    if (cantidad && /(?:HORAS?|H)$/.test(unidad)) return frio ? { taller: frio } : null;
    if (cantidad && /KM$/.test(unidad)) return { taller: vehiculo };
    if (cantidad && km) {
      if (cantidad <= km / 10) return frio ? { taller: frio } : null;
...
function metrogestionLeerTrabajos_(
  ...
  const result = [];
  const referenciasKm = metrogestionReferenciasKilometros_(values);
  for (let index = 1; index < values.length; index += 1) {
    const row = values[index];
    const dfm = metrogestionNormalizar_(row[0]);
    const designacion = metrogestionNormalizar_(row[7]);
    ...
    result.push({
      ...
      taller: destino?.taller || row[5],
      tipo_trabajo: destino?.tipo || tipoTrabajo,
      pedido,
...
// Read-only feed: all pending needs, even without a Hotel visit, date or priority.
// It must never be passed to the stage/order importer.
function metrogestionLeerPendientesListado_(values, workNotes, workBackgrounds, orderBackgrounds) {
  const ignored = new Set(['ALTA', 'BAJA', 'PARADA', 'ANULADA', 'FIN', 'ARCHIVO',
    'CARPETA', 'PRIMITIVA', 'TANCAMENT', 'MITJANA', 'CANVI']);
  const result = [];
  for (let index = 1; index < values.length; index += 1) {
    const row = values[index];
    const dfm = metrogestionNormalizar_(row[0]);
    ...
    result.push({
      ...
      clave_fila: metrogestionClaveFilaTrabajo_(row, order), numero_parada: stop,
      taller: row[5], tipo_trabajo: order ? '' : row[6], designacion: row[7],
      detalle: row[16], fecha_necesidad: metrogestionFechaIso_(row[8], 'de necesidad'),
      fecha_realizada: metrogestionFechaIso_(row[9], 'de realización'),
      fecha_recogida: '', pendiente_fondo_blanco: white,
    });
  }
  return result;
}
...
// Resolver exclusivamente contra la hoja actual. La fila recibida del servidor
// es orientativa: insertarla, moverla o filtrar no cambia la identidad del trabajo.
function metrogestionResolverAsignacion_(assignment, rows, expectedStop, usedRows) {
  const syncId = String(assignment.trabajo_sync_id || '').toLowerCase();
  const expectedKey = String(assignment.clave_fila || '').trim();
  const identity = rows.filter(r => metrogestionCoincideIdentidadTrabajo_(r.values, expectedKey));
  const available = identity.filter(r => !usedRows.has(r.rowNumber));
  const linked = available.filter(r => r.syncId === syncId);
  const sameStop = available.filter(r => !r.syncId && r.stop === expectedStop);
  const pending = available.filter(r => !r.syncId && r.pending && r.key === expectedKey);
  let candidates = linked.length ? linked : sameStop.length ? sameStop : pending;
  // F/G pueden haberse completado después del enlace. Si hay varias líneas
  // del mismo trabajo, su clave completa distingue las que siguen coincidiendo.
  const exact = candidates.filter(r => r.key === expectedKey);
  if (exact.length) candidates = exact;
  if (candidates.length > 1) {
    throw new Error(`Hay varias filas con la identidad de la necesidad procedente de la fila ${assignment.fila || 'desconocida'}. Revisa sus vínculos antes de continuar.`);
  }
  const found = candidates[0];
  // Una referencia retirada o sustituida por otro UUID no autoriza a escribir
  // sobre la fila que ahora ocupa su posición ni sobre una nota copiada a otra H.
  if (!found) return { rowNumber: 0, syncId, alreadyLinked: true };
  if (found.stop && found.stop !== expectedStop) {
    throw new Error(`La fila ${found.rowNumber} ya pertenece a otra actuación. No se ha reasignado.`);
  }
  usedRows.add(found.rowNumber);
  return { rowNumber: found.rowNumber, syncId, alreadyLinked: found.syncId === syncId && found.stop === expectedStop };
}
...
function metrogestionAplicarAsignacionesTrabajos_(
  ...
  // Releer tras cualquier inserción de PARADA de esta misma orden. Nunca usar
  // los números de fila guardados al principio del ciclo como posición actual.
  const count = sheetState?.values?.length || sheet.getLastRow();
  const values = sheetState?.values || sheet.getRange(1, 1, count, 17).getDisplayValues();
  const notesE = sheetState?.notesE || sheet.getRange(1, 5, count, 1).getNotes().map(r => String(r[0] || ''));
  const colorsG = sheet.getRange(1, 7, count, 1).getBackgrounds();
  const liveWorks = metrogestionLeerTrabajos_(values, notesE.map(note => [note]),
    sheet.getRange(1, 8, count, 1).getBackgrounds(),
    sheet.getRange(1, 1, count, 1).getBackgrounds(),
    metrogestionFechaCorteTrabajos_(), colorsG);
  const pendingRows = new Set(liveWorks.map(work => Number(work.fila)));
  const rows = values.slice(1).map((row, i) => ({
    rowNumber: i + 2, values: row,
    key: metrogestionClaveFilaTrabajo_(row, metrogestionEsFondoAmarillo_(colorsG[i + 1]?.[0])),
    syncId: metrogestionNotaTrabajoId_(notesE[i + 1]).toLowerCase(),
    stop: metrogestionNormalizar_(row[4]).replace(/^PA-/, ''),
    pending: pendingRows.has(i + 2),
  }));
  ...
  const planned = assignments.map(assignment => {
    ...
    return metrogestionResolverAsignacion_(assignment, rows, expectedStop, usedRows);
  });
...
// Motor de próximas necesidades conservado del script activo alpha75-2026.09.17.4.
...
function metrogestionPlanificarNecesidades_(records, today) {
  const referenciasKm = metrogestionReferenciasKilometros_(records.map(r => r.values));
  ...
  const linkNext = (source, type, nextDate, cat, mDate) => {
    ...
    if (!child) {
      ...
      next[7] = type;
      next[8] = nextDate;
      const destino = metrogestionReglaTallerPendiente_(next, referenciasKm);
      if (destino) {
        next[5] = destino.taller;
        if (destino.tipo) next[6] = destino.tipo;
      }
  ...
  rows.forEach(r => {
    ...
    if (metrogestionEsFondoBlanco_(r.colorH) && !metrogestionEsFondoAmarillo_(r.colorG)) {
      const destino = metrogestionReglaTallerPendiente_(r.values, referenciasKm);
      if (destino && (String(r.values[5] || '').trim() !== destino.taller
          || (destino.tipo && metrogestionNormalizar_(r.values[6]) !== metrogestionNormalizar_(destino.tipo)))) {
        const c = change(r);
        c.values[5] = destino.taller;
        if (destino.tipo) c.values[6] = destino.tipo;
        if (r.meta.generada && r.meta.firma === currentSignature(r)) {
          writeMeta(r, { ...r.meta, firma: metrogestionFirmaNecesidad_(c.values) });
        }
      }
    }
...
function metrogestionActualizarCiclo_(sheet, state, modo, token) {
  if (state.version === METROGESTION.scriptVersion || state.confirmacionPendiente) return false;
  if (state.fase === 'comandos') {
    metrogestionRegistrarPaso_('Actualizando las órdenes pendientes con el servidor');
    const { syncResult } = metrogestionSolicitarCiclo_(sheet, modo, token);
    if (!Array.isArray(syncResult.comandos_manteniment)) throw new Error('El servidor no devolvió la cola de órdenes pendientes. Se conserva el avance anterior.');
    // La cola del servidor incluye las revisiones corregidas y conserva allí
    // las órdenes pendientes que excedan el límite de una respuesta.
    state.comandos = syncResult.comandos_manteniment;
    state.totalComandos = state.confirmados + state.comandos.length;
...
function metrogestionProcesarOrdenesLote_(sheet, state, token, deadline) {
  // Si la hoja ya quedó escrita, un fallo de red solo debe repetir el acuse.
  if (state.confirmacionPendiente) {
    metrogestionRegistrarPaso_('Reintentando la confirmación de la orden ya aplicada');
    metrogestionConfirmarComandos_(token, [state.confirmacionPendiente]);
    state.comandos.shift();
    state.confirmados += 1;
    delete state.confirmacionPendiente;
    if (!state.comandos.length) state.fase = 'necesidades';
    metrogestionGuardarCiclo_(state);
    return;
  }
  ...
  try {
    ...
    while (state.comandos.length && processed < METROGESTION_LOTES.comandos && Date.now() < deadline) {
      ...
      const confirmation = metrogestionAplicarComandos_(sheet, [command], sheetState, [])[0];
      metrogestionRegistrarPaso_('Confirmando en la hoja la orden aplicada');
      SpreadsheetApp.flush();
      state.confirmacionPendiente = confirmation;
      metrogestionGuardarCiclo_(state);
      metrogestionRegistrarPaso_('Confirmando la orden aplicada con el servidor');
    ...
    // El restablecimiento no debe ocultar el paso ni el error originales.
    try {
      if (!failure) metrogestionRegistrarPaso_('Restaurando el filtro después de las órdenes');
      metrogestionRestaurarFiltro_(sheet, filterState);
    } catch (filterError) {
      if (!failure) throw filterError;
      failure.message += ` Además, no se pudo restaurar el filtro: ${filterError.message}`;
...
function metrogestionEjecutarSincronizacion_(modo, expectedCycle) {
  ...
  try {
    ...
    if (!state || state.fase === 'terminado') {
      ...
      state = { id: Utilities.getUuid(), hoja: METROGESTION.spreadsheetId, fase: commands.length ? 'comandos' : 'necesidades', comandos: commands, trabajos: [], version: METROGESTION.scriptVersion, totalComandos: commands.length, confirmados: 0, creadas: 0, cierres: 0, mensajeBase: syncResult.mensaje || 'Sincronización correcta.' };
      metrogestionGuardarCiclo_(state);
      // La lectura y el intercambio con Supabase tienen su propia ejecución.
      return metrogestionRespuestaCiclo_(state);
    }
    // Migrar una cola antigua en una llamada separada, sin borrar su ID ni sus contadores.
    if (metrogestionActualizarCiclo_(sheet, state, modo, token)) return metrogestionRespuestaCiclo_(state);
    if (state.fase === 'comandos') {
