...
function stageCard(row, today) {
  ...
  const meta = [vehicle, row.numero_parada || 'Sin n.º de actuación', row.lugar || row.taller].filter(Boolean).join(' · ');
  const dateLabel = row.source === 'manteniment' ? 'Fecha de caducidad (MANTENIMENT · columna I)' : 'Fecha prevista (Hotel)';
  const dateText = date ? formatDate(date) : 'Sin fecha';
  card.append(head, el('p', meta, 'a74-pending-meta'), el('p', `${dateLabel}: ${dateText}`, 'a74-pending-date'));
...
function exportTable(rows) {
  const headers = ['Origen', 'DFM', 'Matrícula', 'N.º actuación', 'T / fila', 'Tipo', 'Trabajo', 'Taller', 'Fecha prevista Hotel', 'Caducidad MANTENIMENT (I)', 'Estado'];
  const values = rows.map(row => [
    ...
    familyLabel(row.familia), row.trabajos || row.nombre || '', row.lugar || row.taller || '',
    row.source === 'manteniment' ? '' : formatDate(row.fecha_referencia),
    row.source === 'manteniment' ? formatDate(row.fecha_referencia) : '',
    stateLabel(row.estado),
  ]);
