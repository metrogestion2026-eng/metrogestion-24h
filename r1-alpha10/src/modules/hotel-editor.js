...
export async function openHotelEditor(registroId, { onSaved } = {}) {
  ...
  const overlay = element('div', {
    className: 'hotel-editor-overlay', role: 'dialog', 'aria-modal': 'true', 'aria-label': 'Edición completa de Hotel'
  });
  ...
  const header = element('header', { className: 'hotel-editor-header' }, [
    element('div', {}, [
      element('p', { className: 'eyebrow', text: 'Ficha piloto · guardado único y auditado' }),
      title
    ]),
  ...
  status.textContent = 'Cargando ficha, T, trabajos y catálogos…';
  const { data, error } = await supabase.rpc('obtener_ficha_hotel_edicion', {
    p_registro_id: registroId
  });

  if (error || !data) {
    status.className = 'hotel-editor-status error';
    status.textContent = error?.message || 'No se pudo cargar la ficha de edición.';
    return;
  }
  ...
  saveButton.addEventListener('click', async () => {
    ...
    status.className = 'hotel-editor-status';
    status.textContent = 'Guardando toda la ficha en una única transacción y registrando la auditoría…';
    ...
    const { data: saved, error: saveError } = await supabase.rpc('guardar_ficha_hotel_edicion', {
      ...
      p_etapas: stagesPayload(detail.etapas),
    ...
    dirty = false;
    detail = normalizeDetail(saved.detalle);
    versionBadge.textContent = `Versión ${saved.version}`;
    status.className = 'hotel-editor-status success';
    status.textContent = Number(saved.eventos_auditoria) === 0
      ? '✓ No había cambios reales. No se creó ningún registro de auditoría.'
      : `✓ Ficha guardada. ${saved.eventos_auditoria} cambio(s) registrados con la referencia ${saved.request_id}.`;

    await onSaved?.();
    saveButton.textContent = 'Guardado correctamente';
  ...
  body.append(form);
  status.className = 'hotel-editor-status success';
  status.textContent = 'Ficha cargada. Nada se modifica hasta pulsar “Guardar ficha completa”.';
}
